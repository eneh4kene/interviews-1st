...
import { jobAggregationService } from '../../web/src/lib/services/jobAggregation';
...
export class JobDiscoveryService {
    ...
    async getFilteredJobsForClient(clientId: string, filters: JobSearchFilters = {}): Promise<ClassifiedJobListing[]> {
        try {
            ...
            // 4. Classify jobs for AI applicability
            const classifiedJobs = await jobClassificationService.batchClassifyJobs(jobs);

            // 5. Create classified job listings with match scores
            const classifiedJobListings: ClassifiedJobListing[] = jobs.map(job => {
                const classification = classifiedJobs.find(c => c.job_id === job.id);
                const matchPercentage = this.calculateMatchPercentage(job, clientPreferences);

                return {
                    ...job,
                    is_ai_applicable: classification?.is_ai_applicable || false,
                    confidence_score: classification?.confidence_score || 0,
                    classification_reasons: classification?.classification_reasons || [],
                    application_method: classification?.application_method || 'manual',
